...
`scripts/ste-lint.py` checks the mechanical subset of these rules (long sentences, semicolons, contractions, passive voice, banned words, marketing adjectives, phrasal verbs, em dashes). Score is violations per 100 words, lower is cleaner. It has no third-party dependencies, so `uv run` executes it with no setup.

Run it before and after applying this skill to a draft — the score delta is the signal:

```
uv run .claude/skills/ste-writing/scripts/ste-lint.py your-draft.md
```

It is not a certified STE checker. It is a deterministic proxy for the machine-checkable part of the standard.
